...
func scanSubject(sc scanner) (meta.Subject, error) {
	var (
	...
	subject.CreatedAt = asTime(created)
	return subject, nil
}
...
func (s *Store) CreateSubject(ctx context.Context, subject meta.Subject) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		nameTaken, err := exists(ctx, tx, `SELECT 1 FROM subjects WHERE name = ?`, subject.Name)
		if err != nil {
			return err
		}
		if nameTaken {
		...
		idTaken, err := exists(ctx, tx, `SELECT 1 FROM subjects WHERE id = ?`, subject.ID)
		if err != nil {
			return err
		}
		if idTaken {
		...
		_, err = execute(ctx, tx,
			...
			subject.ID, subject.Name, string(subject.Kind), subject.Disabled, millis(subject.CreatedAt))
		return err
	})
}
...
func (s *Store) subject(ctx context.Context, q querier, name string) (meta.Subject, error) {
...
func (s *Store) SetSubjectDisabled(ctx context.Context, name string, disabled bool) error {
	...
	affected, err := execute(ctx, s.db, `UPDATE subjects SET disabled = ? WHERE name = ?`, disabled, name)
	if err != nil {
		return err
	}
	if affected == 0 {
...
func (s *Store) DeleteSubject(ctx context.Context, name string) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		...
		_, err = execute(ctx, tx, `DELETE FROM subjects WHERE name = ?`, name)
		return err
	})
}
...
func scanGroup(sc scanner) (meta.SubjectGroup, error) {
	var (
	...
	group.CreatedAt = asTime(created)
	return group, nil
}
...
func (s *Store) CreateGroup(ctx context.Context, group meta.SubjectGroup) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		taken, err := exists(ctx, tx, `SELECT 1 FROM subject_groups WHERE name = ? OR id = ?`,
			group.Name, group.ID)
		if err != nil {
			return err
		}
		...
		_, err = execute(ctx, tx,
			...
			group.ID, group.Name, millis(group.CreatedAt))
		return err
	})
}
...
func (s *Store) group(ctx context.Context, q querier, name string) (meta.SubjectGroup, error) {
...
func (s *Store) ListGroups(ctx context.Context) ([]meta.SubjectGroup, error) {
	...
	return collect(ctx, s.db, `SELECT id, name, created_at FROM subject_groups ORDER BY name`, nil,
...
func (s *Store) DeleteGroup(ctx context.Context, name string) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		...
		_, err = execute(ctx, tx, `DELETE FROM subject_groups WHERE name = ?`, name)
		return err
	})
}
...
func (s *Store) CreateRole(ctx context.Context, role meta.Role) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		taken, err := exists(ctx, tx, `SELECT 1 FROM roles WHERE name = ?`, role.Name)
		if err != nil {
			return err
		}
		if taken {
		...
		if _, err := execute(ctx, tx, `INSERT INTO roles (name, builtin) VALUES (?, ?)`,
...
func (s *Store) role(ctx context.Context, q querier, name string) (meta.Role, error) {
	...
	verbs, err := collect(ctx, q, `SELECT verb FROM role_verbs WHERE role_name = ? ORDER BY verb`,
		[]any{name},
...
func (s *Store) UpdateRoleVerbs(ctx context.Context, name string, verbs []string) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		...
		if _, err := execute(ctx, tx, `DELETE FROM role_verbs WHERE role_name = ?`, name); err != nil {
			return err
		}
...
func (s *Store) DeleteRole(ctx context.Context, name string) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		...
		_, err = execute(ctx, tx, `DELETE FROM roles WHERE name = ?`, name)
		return err
	})
}
...
func scanBinding(sc scanner) (meta.Binding, error) {
	var (
	...
	binding.CreatedAt = asTime(created)
	return binding, nil
}
...
func (s *Store) CreateBinding(ctx context.Context, binding meta.Binding) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		taken, err := exists(ctx, tx, `SELECT 1 FROM bindings WHERE id = ?`, binding.ID)
		if err != nil {
			return err
		}
		if taken {
		...
		_, err = execute(ctx, tx,
			...
			binding.Role, binding.Scope, millis(binding.CreatedAt))
		return err
	})
}
...
func principalExists(ctx context.Context, tx *sql.Tx, kind meta.PrincipalKind, id string) (bool, error) {
	...
	case meta.PrincipalGroup:
		return exists(ctx, tx, `SELECT 1 FROM subject_groups WHERE id = ?`, id)
	default:
		return exists(ctx, tx, `SELECT 1 FROM subjects WHERE id = ?`, id)
	}
}
...
func (s *Store) ListBindings(ctx context.Context) ([]meta.Binding, error) {
	...
	return collect(ctx, s.db, `SELECT `+bindingColumns+` FROM bindings ORDER BY id`, nil,
...
func (s *Store) DeleteBinding(ctx context.Context, id string) error {
	...
	affected, err := execute(ctx, s.db, `DELETE FROM bindings WHERE id = ?`, id)
	if err != nil {
		return err
	}
	if affected == 0 {
...
func (s *Store) ListEffectiveBindings(ctx context.Context, subjectName string) ([]meta.EffectiveBinding, error) {
	...
	return collect(ctx, s.db,
		...
		func(rows *sql.Rows) (meta.EffectiveBinding, error) {
			...
			effective.CreatedAt = asTime(created)
