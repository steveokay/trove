...
func (t *tx) AppendEvent(ctx context.Context, event meta.Event) error {
	...
	taken, err := sqlutil.Exists(ctx, t.tx, `SELECT 1 FROM events WHERE id = $1`, event.ID)
	if err != nil {
		return err
	}
	if taken {
	...
	// Two writers can pass the existence check at once; the primary key
	// catches the second, and the caller should see the same ErrConflict a
	// serialised store would have returned.
	return asConflict(err, "event", event.ID)
}
...
func (s *Store) ListEvents(ctx context.Context, opts meta.ListOptions) (meta.EventPage, error) {
	...
	events, err := sqlutil.Collect(ctx, s.db,
		`SELECT `+eventColumns+` FROM events
		 WHERE `+where+` AND id > `+cursorArg+` ORDER BY id LIMIT `+limitArg,
		args, scanEventRow)
	if err != nil {
		return meta.EventPage{}, err
	}
