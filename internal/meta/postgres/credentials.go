...
func (s *Store) PutUserCredential(ctx context.Context, cred meta.UserCredential) error {
	...
	return sqlutil.InTx(ctx, s.db, func(tx *sql.Tx) error {
		...
		_, err := sqlutil.Execute(ctx, tx,
			...
			cred.Subject, cred.Hash, cred.MustRotate, sqlutil.Millis(cred.RotatedAt))
		return err
	})
}
		...
		_, err = sqlutil.Execute(ctx, tx,
			`INSERT INTO robot_credentials (subject_name, secret_hash, expires_at, rotated_at)
			 ...
			 ON CONFLICT (subject_name) DO UPDATE SET
			     ...
			     rotated_at = excluded.rotated_at`,
			cred.Subject, cred.SecretHash, cred.ExpiresAt.UTC().UnixMilli(), sqlutil.Millis(cred.RotatedAt))
		return err
	})
}
...
func scanAccessToken(sc sqlutil.Scanner) (meta.AccessToken, error) {
	var (
	...
	token.CreatedAt = sqlutil.AsTime(created)
	token.ExpiresAt = sqlutil.AsTime(expires)
	token.LastUsedAt = sqlutil.AsTime(used)
	return token, nil
}
...
func scanSession(sc sqlutil.Scanner) (meta.Session, error) {
	var (
	...
	session.CreatedAt = sqlutil.AsTime(created)
