...
| F-005b | `meta.Store` identity + authz groups | review | F-005 ✓ | Subjects (incl. undeletable anonymous), local groups, roles with read-only built-ins, bindings, and `ListEffectiveBindings` with group provenance; 14 added contract cases, 164 subtests total. Coverage 98.7%. Credentials/sessions deferred to F-005c |
| F-005c | `meta.Store` credentials + sessions | todo | F-005b | Argon2id user credentials, robot secrets, personal access tokens, UI sessions; hashed at rest, same suite |
