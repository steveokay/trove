...
| F-005 | `meta.Store` core: repositories + content | review | D-006 ✓ | `internal/meta` types/errors/Visibility, `metatest` 24-case contract suite incl. exhaustive per-method cancellation, `memory` reference impl green under `-race`. Coverage 98.6%. Amends §9: shared test harnesses excluded from the gate |
| F-005b | `meta.Store` identity + authz groups | todo | F-005 | Subjects, groups, roles, bindings, credentials, sessions; same suite |
