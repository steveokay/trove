...
| F-004 | Structured logging + graceful shutdown | review | F-001 ✓, F-003 ✓ | `internal/server`: slog JSON/text by config, per-request IDs echoed in `Trove-Request-Id` and carried on a context logger, access log levelled by status; graceful drain proven by an in-flight-request test; ADR 0018 data-dir lock (flock/Windows `CreateFile`) with second-start refusal tested; `trove serve` wired with SIGTERM/SIGINT handling. Coverage 97.8% |
