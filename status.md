...
| C-013 | Cache eviction (size/LRU budget) | in-progress | D-013 ✓, Q11 ✓, C-004 ✓ | **Store layer landed** (commit 1 of 2): `CachedUsage` (sums rows, not the disk; entity carve-outs matched by name *range* so `quay` never reaches `quay2`), `ListEvictable` (manifests and blobs in one LRU order since they compete for one budget; the union is wrapped because neither engine allows an expression in an ORDER BY over a union, and the key is COALESCE(last_access_at,0) because SQLite sorts NULL first and Postgres last), `DeleteCachedBlob` **returning the claims that remain** (shared content-addressed bytes must not be reclaimed while another proxy still holds a row — delete and count share a transaction so no claim can appear in between), `CachedBlobClaims` for the orphan sweep, `DeleteCachedManifest`, and `TouchCached` (the batched LRU write C-004 deferred, monotonic, and an access for evicted content resurrects nothing). **Next: `internal/cache`** — the sweep (global budget + per-proxy carve-outs, breach trigger, schedule), `cache.evicted` events, the orphan pass, the touch batcher, and ADR 0009 proving test (d), which needs an eviction pass to observe. CI on the store-layer commit (52d4a46) failed on an unrelated MinIO restart-readiness flake in `internal/blob/s3`, fixed in the following commit (§9: a flake is fixed the day it flakes). Details → `docs/notes/phase-4.md` |
