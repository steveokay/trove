...
| R-010 | Pull statistics: last-pulled + counts | review | R-002 ✓ | `meta.RecordPulls`/`GetPullStats` (all three engines, migration 0003, contract cases incl. the failure tables) + `registry.PullBatcher` on `Manifests.Pulls`. Proven: 50 GETs through the real stack → **zero** store calls, one batched upsert on flush. Counts accumulate, last-pulled never regresses, no FK so a repointed tag cannot fail a flush; HEAD deliberately not counted. `artifact.pulled` event waits on E-001. Details → `docs/notes/phase-3.md` |
